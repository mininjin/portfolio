<template>
  <div class="fixed bottom-0 md:bottom-auto md:top-0 left-0 w-full z-40 text-primary shadow">
    <div class="flex w-full bg-highlight items-center">
      <div class="p-2 text-left flex-auto text-3xl font-title">{{ APP_NAME }}</div>
      <div class="p-2 flex justify-center items-center cursor-pointer" @click="toggleMenu">
        <MenuButton :on="menu" />
      </div>
    </div>
  </div>
  <transition enter-from-class="translate-y-full md:-translate-y-full"
    leave-to-class="translate-y-full md:-translate-y-full">
    <div v-if="menu"
